...
def update_label1(direction, value):
    global V_value_label1, H_value_label1
    if direction in ["Up" ,"Down"]:
        V_value_label1.setText(str(value))
    if direction in ["Left", "Right"]:
        H_value_label1.setText(str(value))


def update_label2(direction, value):
    global V_value_label2, H_value_label2
    if direction in ["Up" ,"Down"]:
        V_value_label2.setText(str(value))
    if direction in ["Left", "Right"]:
        H_value_label2.setText(str(value))
...
if __name__ == "__main__":
    ...
    ## 생성된 조이스틱을 위젯에 추가
    ml.addWidget(joy1, 0, 0)
    ml.addWidget(joy2, 0, 2)

    # 각도를 표시할 레이블 생성 및 추가
    global V_value1, H_value1
    V_label1 = QLabel("Bottom")
    ml.addWidget(V_label1, 1, 0)
    V_value_label1 = QLabel(str(joy1.vertical))
    ml.addWidget(V_value_label1, 2, 0)

    H_label = QLabel("Sholder")
    ml.addWidget(H_label, 1, 1)
    H_value_label1 = QLabel(str(joy1.horizon))
    ml.addWidget(H_value_label1, 2, 1)


    # 각도를 표시할 레이블 생성 및 추가
    global V_value2, H_value2
    V_label2 = QLabel("Elbow")
    ml.addWidget(V_label2, 1, 2)
    V_value_label2 = QLabel(str(joy2.vertical))
    ml.addWidget(V_value_label2, 2, 2)

    H_label2 = QLabel("Gripper")
    ml.addWidget(H_label2, 1, 3)
    H_value_label2 = QLabel(str(joy2.horizon))
    ml.addWidget(H_value_label2, 2, 3)

    ## Joystic의 angleChange 신호가 발생할때마다 
    ## update_label 함수가 호출되어 레이블 업데이트
    joy1.angleChange.connect(update_label1)
    joy2.angleChange.connect(update_label2)
